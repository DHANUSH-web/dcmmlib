...
TEST_F(VsCodeHome, ExtensionsFirstWhenPresent) {
  fs::create_directories(home / ".vscode" / "extensions" / "pub.ext-1.0.0");
  fs::create_directories(home / ".vscode-shared");
  {
    std::ofstream((home / ".vscode" / "extensions" / "pub.ext-1.0.0" / "package.json").string())
        << R"({"displayName":"Pub Ext"})";
    std::ofstream((home / ".vscode" / "extensions" / "pub.ext-1.0.0" / "icon.png").string())
        << "png";
  }
  auto items = dcmm::vsCodeItems(dcmm::VsCodeEdition::Stable, home.string());
...
TEST_F(VsCodeHome, PlaceholderDisplayNameUsesFolder) {
  ...
  auto exts = dcmm::vsCodeExtensions(dcmm::VsCodeEdition::Stable, home.string());
  ASSERT_EQ(exts.size(), 1u);
  EXPECT_EQ(exts[0].name, "pub.thing-1.0.0");
}
